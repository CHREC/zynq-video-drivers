...
to_psee_entity(struct v4l2_async_connection *asd)
{
		       ...
		       const struct fwnode_handle *fwnode)
{
	...
	struct list_head *lists[] = {
		&pdev->notifier.done_list,
		&pdev->notifier.waiting_list
	};
	unsigned int i;
...
static int psee_graph_notify_complete(struct v4l2_async_notifier *notifier)
{
	...
	list_for_each_entry(asd, &pdev->notifier.done_list, asc_entry) {
		entity = to_psee_entity(asd);
...
{
	struct psee_graph_entity *entity = to_psee_entity(asc);
				...
				struct fwnode_handle *fwnode)
{
	...
	while (1) {
		...
		xge = v4l2_async_nf_add_fwnode(&pdev->notifier, remote,
					       struct psee_graph_entity);
		fwnode_handle_put(remote);
		if (IS_ERR(xge)) {
			ret = PTR_ERR(xge);
...
err_notifier_cleanup:
	v4l2_async_nf_cleanup(&pdev->notifier);
	fwnode_handle_put(ep);
	return ret;
}
...
static int psee_graph_parse(struct psee_composite_device *pdev)
{
	...
	list_for_each_entry(asd, &pdev->notifier.waiting_list, asc_entry) {
		entity = to_psee_entity(asd);
...
static void psee_graph_cleanup(struct psee_composite_device *pdev)
{
	...
	v4l2_async_nf_unregister(&pdev->notifier);
	v4l2_async_nf_cleanup(&pdev->notifier);
...
printk(KERN_WARNING "COMPOSITE GRAPH 4\n");
	...
	ret = v4l2_async_nf_register(&pdev->notifier);
	if (ret < 0) {
...
static void psee_composite_remove(struct platform_device *platform_dev)
{
...
static struct platform_driver psee_composite_driver = {
	...
	.remove_new = psee_composite_remove,
};
